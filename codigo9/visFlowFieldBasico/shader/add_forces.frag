...
// -------------------------------
// shaders/add_forces.frag
// -------------------------------
#version 410 core
in vec2 vUV; out vec2 outVel;
uniform sampler2D uVel;
uniform float uTime, uDt;
uniform float uForceAmp; // e.g. 0.5
uniform float uCurlScale; // e.g. 2.0
...
float fbm3(vec3 p){
    ...
    for(int i=0;i<5;i++){ 
        v += amp*noise3(p*freq); amp*=0.5; freq*=2.0; 
        }
    return v;
}
...
// Zero normal at borders (simple)
    ...
    if(uv.x>=0.999) vel.x = min(vel.x, 1.0);
    if(uv.y<=0.001) vel.y = max(vel.y, 0.0);
    if(uv.y>=0.999) vel.y = min(vel.y, 1.0);
    return vel;
}

void main(){
    vec2 v = texture(uVel, vUV).xy;
    vec2 f = curlNoise2D(vUV, uTime, uCurlScale, 0.01) * uForceAmp;
    v += uDt * f;
    outVel = enforceBorders(v, vUV);
}